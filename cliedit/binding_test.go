...
func setup(cfg *cli.AppConfig) (*cli.App, clicore.TTYCtrl, clicore.SignalSourceCtrl) {
	tty, ttyControl := clicore.NewFakeTTY()
	sigs, sigsCtrl := clicore.NewFakeSignalSource()
	app := cli.NewApp(cfg, tty, sigs)
...
/*
func TestMapBinding_CallsBinding(t *testing.T) {
	called := 0
	m := buildBinding(
		"a", eval.NewGoFn("[test]", func() { called++ }),
		"Enter", eval.NewGoFn("[default]", cli.DefaultInsert))
	binding := newMapBinding(eval.NewEvaler(), &m)

	app, tty, _ := setup(&cli.AppConfig{
		InsertModeConfig: cli.InsertModeConfig{Binding: binding}})
...
func TestMapBinding_KeyHandler_CallsBinding(t *testing.T) {
	called := 0
	m := buildBinding(
		"a", eval.NewGoFn("[test]", func() { called++ }))
	binding := newMapBinding(eval.NewEvaler(), &m)

	handler := binding.KeyHandler(ui.K('a'))
	// TODO: Pass actual KeyHandler
	handler(nil)
