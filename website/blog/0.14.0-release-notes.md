...
-   The `type` and `path` fields of the value returned by `src` have been
    removed. Instead, its `name` field now contains the full path of the file,
    when the source originates from a file.
