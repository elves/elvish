...
func TestGetVerticalWindow(t *testing.T) {
	tt.Test(t, tt.Fn("getVerticalWindow", getVerticalWindow), tt.Table{
		...
		Args(State{Items: TestItems{NItems: 10}, Selected: 0, First: 0}, 6).Rets(0, 0),
		Args(State{Items: TestItems{NItems: 10}, Selected: 0, First: 1}, 6).Rets(0, 0),
		// selected < 0 is treated as if = 0.
		Args(State{Items: TestItems{NItems: 10}, Selected: -1, First: 0}, 6).Rets(0, 0),
		// selected = n-1: always show a window ending at n-1, regardless of the
		// value of oldFirst
		Args(State{Items: TestItems{NItems: 10}, Selected: 9, First: 0}, 6).Rets(4, 0),
		Args(State{Items: TestItems{NItems: 10}, Selected: 9, First: 8}, 6).Rets(4, 0),
		// selected >= n is treated as if = n-1.
		Args(State{Items: TestItems{NItems: 10}, Selected: 10, First: 0}, 6).Rets(4, 0),
		// selected = 3, oldFirst = 2 (likely because previous selected = 4).
		// Adjust first -> 1 to satisfy the upward respect distance of 2.
		Args(State{Items: TestItems{NItems: 10}, Selected: 3, First: 2}, 6).Rets(1, 0),
		// selected = 6, oldFirst = 2 (likely because previous selected = 7).
		// Adjust first -> 3 to satisfy the downward respect distance of 2.
		Args(State{Items: TestItems{NItems: 10}, Selected: 6, First: 2}, 6).Rets(3, 0),

		// There is not enough budget to achieve respect distance on both sides.
		// Split the budget in half.
		Args(State{Items: TestItems{NItems: 10}, Selected: 3, First: 1}, 3).Rets(2, 0),
		Args(State{Items: TestItems{NItems: 10}, Selected: 3, First: 0}, 3).Rets(2, 0),

		// There is just enough distance to fit the selected item. Only show the
		// selected item.
		Args(State{Items: TestItems{NItems: 10}, Selected: 2, First: 0}, 1).Rets(2, 0),
	})
}
...
func TestGetHorizontalWindow(t *testing.T) {
	tt.Test(t, tt.Fn("getHorizontalWindow", getHorizontalWindow), tt.Table{
		...
		Args(State{Items: TestItems{NItems: 10}, Selected: 4, First: 0}, 0, 6, 10).Rets(0, 10),
		// All items fit in multiple columns. Item width is 2 ("x0").
		Args(State{Items: TestItems{Prefix: "x", NItems: 10}, Selected: 4, First: 0}, 0, 6, 5).Rets(0, 5),
		// All items cannot fit, selected = 0; show a window from 0. Height
		// reduced to make room for scrollbar.
		Args(State{Items: TestItems{Prefix: "x", NItems: 11}, Selected: 0, First: 0}, 0, 6, 5).Rets(0, 4),
		...
		Args(State{Items: TestItems{Prefix: "x", NItems: 11}, Selected: 10, First: 0}, 0, 7, 5).Rets(4, 4),
		// Items are wider than terminal, and there is a single column. Show
		// them all.
		Args(State{Items: TestItems{Prefix: "long prefix", NItems: 10}, Selected: 9, First: 0}, 0,
			6, 10).Rets(0, 10),
		...
		Args(State{Items: TestItems{Prefix: "x", NItems: 20}, Selected: 10, First: 0}, 0, 8, 6).Rets(5, 5),
		// First = 2. Ditto.
		Args(State{Items: TestItems{Prefix: "x", NItems: 20}, Selected: 10, First: 2}, 0, 8, 6).Rets(5, 5),
		// First = 5. Show columns 5-9 and 10-14.
		Args(State{Items: TestItems{Prefix: "x", NItems: 20}, Selected: 10, First: 5}, 0, 8, 6).Rets(5, 5),
		// First = 7. Ditto.
		Args(State{Items: TestItems{Prefix: "x", NItems: 20}, Selected: 10, First: 7}, 0, 8, 6).Rets(5, 5),
		// First = 10. No need to any columns to the left.
		Args(State{Items: TestItems{Prefix: "x", NItems: 20}, Selected: 10, First: 10}, 0, 8, 6).Rets(10, 5),
		// First = 12. Ditto.
		Args(State{Items: TestItems{Prefix: "x", NItems: 20}, Selected: 10, First: 12}, 0, 8, 6).Rets(10, 5),
	})
}
