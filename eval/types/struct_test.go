...
var (
	...
	testStruct           = NewStruct(testStructDescriptor, []Value{"lorem", "ipsum"})
	testStruct2          = NewStruct(testStructDescriptor, []Value{"lorem", "dolor"})
)
...
func TestStructMethods(t *testing.T) {
	...
	if foo, err := testStruct.Index("foo"); foo != "lorem" {
